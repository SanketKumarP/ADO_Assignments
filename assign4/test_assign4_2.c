  ...
  for(i = 0; i < 1000; i++)
    {
      ...
      ASSERT_EQUALS_RID(insert[pos], rid, "did we find the correct RID-1?");
    }
  ...
  for(iter = 0; iter < 50; iter++)
      ...
      for(i = 0; i < 1000; i++)
	{
	  ...
	  if (deletes[pos])
	    ...
	    {
	      TEST_CHECK(findKey(tree, key, &rid));
	      ASSERT_EQUALS_RID(insert[pos], rid, "did we find the correct RID-2?");
	    }
	}
...
testIndexScan (void)
{
  ...
  for(iter = 0; iter < 50; iter++)
	...
	{
	  RID expRid = insert[i++];
	  ASSERT_EQUALS_RID(expRid, rid, "did we find the correct RID-3?");
	}
